...
if JAVA_DAP_ACTIVE then
  vim.list_extend(bundles, vim.split(vim.fn.glob(home .. "/tools/vscode-java-test/server/*.jar"), "\n"))
  vim.list_extend(
    bundles,
    vim.split(
      vim.fn.glob(
...
local config = {
  ...
  cmd = {
    ...
    -- vim.fn.glob(home .. "/tools/jdtls/plugins/org.eclipse.equinox.launcher_*.jar"),
    -- ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^                                       ^^^^^^^^^^^^^^
    ...
    -- 💀
    "-configuration",
    -- home .. "/tools/jdtls/config_" .. CONFIG,
